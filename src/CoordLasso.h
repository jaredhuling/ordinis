            ...
            for (j = 0; j < nvars; ++j)
            {
                if (eligible(j))
                {
                    ...
                    if (beta_prev != threshval)
                    {
                        ...
                        if (threshval != 0.0 && eligible_set(j) == 0) eligible_set(j) = 1;
                        //if (threshval == 0.0 && eligible_set(j) == 1 && beta_nz_prev(j) == 0) eligible_set(j) = 0;
                        if (threshval == 0.0 && eligible_set(j) == 1) eligible_set(j) = 0;
                    }
...
public:
    ...
    void init(double lambda_)
    {
        ...
        if (penalty_factor_size < 1)
        {
            for (int j = 0; j < nvars; ++j) if (std::abs(XY(j)) > (cutoff)) eligible_set(j) = 1;
        } else
        {
            for (int j = 0; j < nvars; ++j) if (std::abs(XY(j)) > (cutoff * penalty_factor(j))) eligible_set(j) = 1;
        }
    ...
    int solve(int maxit)
    {
        ...
        while(current_iter < maxit)
        {
            ...
            current_iter++;
            beta_prev = beta;
            ineligible_set = 1 - eligible_set.array();
